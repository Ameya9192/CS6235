...
def getPath(dt, keyword):
    ...
    fileMin = os.path.join(dirHour, pad(dt.minute, 2) + '.json')
    return fileMin
...
def storeItem(item, keyword, lang):
    ...
    stored = 0
    if not checkIfExists(path, item['id_str']):
...
#     tweets = []
    for elem in cls:
        ...
        item = {'id_str': status_id, 'text': text, 'created_at': created_at, 'timestamp_ms': ts, 'user': {'name': username, 'id_str': user_id}}
        storeItem(item, 'landslide', 'en')
